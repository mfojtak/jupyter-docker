...
RUN RUN DEBIAN_FRONTEND="noninteractive" apt-get update && apt-get install -y software-properties-common && \
...
RUN echo 'export PATH=/opt/conda/bin:$PATH' > /etc/profile.d/conda.sh && \
    ...
    /bin/bash ~/anaconda.sh -b -p /opt/conda && \
    rm ~/anaconda.sh && \
    apt remove -y python3.6 python3.5 python2.7 python-minimal && apt autoremove -y && cp /opt/conda/bin/python /usr/bin/python
ENV PATH /opt/conda/bin:$PATH
ENV LD_LIBRARY_PATH /opt/conda/lib:$LD_LIBRARY_PATH
...
RUN curl -fSsL https://github.com/cdr/code-server/releases/download/v3.5.0/code-server-3.5.0-linux-x86_64.tar.gz -o code-server.tar.gz && \
    tar xvzf code-server.tar.gz && \
    ln -s /code-server-3.5.0-linux-x86_64/code-server /usr/bin/code-server
