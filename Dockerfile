...
RUN curl -fSsL https://github.com/cdr/code-server/releases/download/3.2.0/code-server-3.2.0-linux-x86_64.tar.gz -o code-server.tar.gz && \
    tar xvzf code-server.tar.gz && \
    ln -s /code-server-3.2.0-linux-x86_64/code-server /usr/bin/code-server
